...
class LSystem extends BaseLSystem {
 ...
 // Production rules
  ...
  public void iterate() {
    ...
    {
      char currentCharacter = current.charAt(i);
      
      if(rules.containsKey(currentCharacter))
      {
        currentIterationBuffer.append(rules.get(currentCharacter));
      }
      else
      {
        currentIterationBuffer.append(currentCharacter);
      }
    }
  ...
  public void drawLSystem(Turtle t) {
    ...
    // [DONE]: Loop through each character in the iteration string,
    ...
    for (int i = 0; i < currentIteration.length(); i++) {
      ...
      // [DONE]: Implement different l-system vocabulary
      switch (c) {
        case 'F':
          t.forward(dist);
