...
def gcode_move(current_pos, next_pos, feed_rate=None, should_extrude=False):
    ...
    if(should_extrude == True):
        #first claculate the Aout 
        aOut = (layer_height * (extrusion_width - layer_height)) + (math.pi * (layer_height/2) ** 2)
        #Second calculate the Lout 
        lOut = math.sqrt(((next_pos[0] - current_pos[0]) ** 2) + ((next_pos[1] - current_pos[1]) ** 2) + ((next_pos[2] - current_pos[2]) ** 2))
        #calc extrution
        lE = (aOut * lOut)/(math.pi * (filament_diameter/2) ** 2)
        lEE = e_value_to_string(lE)
        print(lEE)
        move_command_str += " " + PARAM_E + lEE #param_e is an E charecter (which is nessesary for the Gcode)
...
def generate_gcode():
    ...
    for i in range(0, len(lines)):
		...
		#checking if current positions Z value is dif than the next step, so the Z value is correct for the next step
        if not are_floats_equal(current_position[2], line_start_position[2]):
            newPos = gcode_move(current_position, (current_position[0], current_position[1], line_start_position[2]), feed_rate=layer_change_feed_rate, should_extrude=False) #don't extrude when you are moving
            #append the to the list of the commands 
            all_move_commands.append(newPos)
            current_position[2] = line_start_position[2] #only update the current position's z value as that is the onl thing we moved 
            
        # step to move to the next line
        if not is_same_pt(ptA = current_position, ptB = line_start_position):#if our next line’s start position != current_position
            newLine = gcode_move(current_position, line_start_position, feed_rate=travel_feed_rate, should_extrude=False) #don't extrude when you are moving
            all_move_commands.append(newLine)
        ...
        actuallyExtrude = gcode_move(line_start_position, line_end_position, feed_rate = extrusion_feed_rate, should_extrude=True)

        # [TODO]: Append the move command across the line segment 
        all_move_commands.append(actuallyExtrude)
